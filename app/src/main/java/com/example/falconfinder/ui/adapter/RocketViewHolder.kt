...
class RocketViewHolder(private val binding: ItemVehicleBinding) :
    ...
    fun bind(vehicleResponseItem: VehicleResponseItem, itemClickListener: ItemClickListener) {
        ...
        binding.rocketNameTv.text =
            vehicleResponseItem.name?.let {
                resource.getString(R.string.vehicle_name).replace(
                    "{vehicle}",
                    it
                )
            }
        ...
        binding.countTv.text = resource.getString(R.string.vehicle_count_count)
            .replace("{count}", vehicleResponseItem.total_no.toString())

        binding.maxDistTv.text = resource.getString(R.string.max_dist)
            .replace("{dist}", vehicleResponseItem.max_distance.toString())

        binding.rocketSpeedTv.text = resource.getString(R.string.speed)
            .replace("{speed}", vehicleResponseItem.speed.toString())
        ...
        if (vehicleResponseItem.isActive) {
            binding.rocketCl.setOnClickListener {
                itemClickListener.onVehicleClickListener(
                    vehicleResponseItem,
                    !binding.rocketCl.isSelected
                )
            }
        } else {
